...
public class JSMinim implements MinimServiceProvider
{
  ...
  public Mixer getOutputMixer()
  {
	...
	private Map getID3Tags(String filename)
	{
		...
		try
		{
			...
			AudioFileFormat baseFileFormat = null; /*reader.getAudioFileFormat(
																							stream,
																							stream.available());
                     */
			stream.close();
   ...
   */
			...
			float sampleRate, int bitDepth)
	{
		...
		if (sdl != null)
		{
			return null;//new JSAudioSynthesizer(sdl, bufferSize);
		}
		return null;
	}
	...
	public AudioRecording getAudioRecordingClip(String filename)
	{
		...
		return null;//new JSAudioRecordingClip(clip, meta);
	}
   ...
   */
			...
			throws UnsupportedAudioFileException, IOException
	{
		...
		// agent
		return null;//MpegAudioFileReaderWorkaround(this).getAudioInputStream(url, null);
	}
			...
			throws UnsupportedAudioFileException, IOException
	{
		...
		catch (Exception iae)
		{
			...
			return null;//new MpegAudioFileReader(this).getAudioInputStream(is);
		}
	}
			...
			AudioInputStream sourceStream)
	{
		...
		catch (IllegalArgumentException iae)
		{
			...
			try
			{
				...
				return null;//new javazoom.spi.mpeg.sampled.convert.MpegFormatConversionProvider().getAudioInputStream(
							//																													targetFormat,
							//																													sourceStream);
			}
