                        ...
                        implements AudioStream
{
  ...
  JSAudioStream(TargetDataLine tdl, int bufferSize)
  {
     ...
     */
    finished = false;
    mono = true;//( buffer.getChannelCount() == 1 );
    int byteBufferSize = 0;//buffer.getByteArrayBufferSize(line.getFormat());
  ...
  public void run()
  {
    ...
    while ( !finished )
    {
      ...
      if ( mono )
      {
        float[] samp = null;//buffer.getChannel(0);
        effect.process(samp);
        listener.samples(samp);
      }
      else
      {
        float[] sampL = null;//buffer.getChannel(0);
        float[] sampR = null;//buffer.getChannel(1);
